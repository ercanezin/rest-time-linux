use gtk4::prelude::*;
use gtk4::{Application, Box, Button, CssProvider, DrawingArea, GestureClick, Label, Orientation, Picture, Window};
...
pub struct BreakOverlayManager {
    ...
    app: Application,
    config: Config,
    last_yoga_index: Rc<Cell<Option<usize>>>,
}
...
impl BreakOverlayManager {
    pub fn new(app: &Application, config: Config) -> Self {
        ...
        Self {
            ...
            app: app.clone(),
            config,
            last_yoga_index: Rc::new(Cell::new(None)),
        }
    }
    ...
    pub fn spawn_overlays(
        ...
        on_postpone: impl Fn(u64) + 'static + Clone,
    ) {
        self.dismiss();
        ...
        for i in 0..n_monitors {
            ...
            if is_primary {
                ...
                // Bottom Control Bar: Postpone Buttons + Hold to Unlock
                let control_bar = Box::new(Orientation::Horizontal, 8);
                control_bar.set_halign(gtk4::Align::Center);
                control_bar.set_valign(gtk4::Align::Center);

                // 3 Postpone Options: +1 Min, +5 Min, +10 Min
                let make_postpone_button = |label_text: &str, secs: u64| -> Button {
                    let btn = Button::builder()
                        .label(label_text)
                        .css_classes(["postpone-btn"])
                        .build();
                    let cb = on_postpone.clone();
                ...
                control_bar.append(&make_postpone_button("⏱ +1 Min", 60));
                control_bar.append(&make_postpone_button("⏱ +5 Min", 300));
                control_bar.append(&make_postpone_button("⏱ +10 Min", 600));
                ...
                if self.config.behavior.strict_hold_to_unlock {
                    ...
                    control_bar.append(&drawing_area);
                }

                root.append(&control_bar);
                win.set_child(Some(&root));
            } else {
                // SECONDARY SCREENS: Clean, solid, distraction-free black screen
                let black_box = Box::new(Orientation::Vertical, 0);
                black_box.add_css_class("break-surface");
                win.set_child(Some(&black_box));
            }
